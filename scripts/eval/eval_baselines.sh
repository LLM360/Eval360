...
for metric_name in ${!metrics[@]}; do
    ...
    for model_path in ${single_node_models[@]}; do
      echo ${model_path}
      if [[ -d ${model_path}/eval_results/${metric_name}_${metrics[${metric_name}]}shots ]]

      then
          echo "eval results for ${model_path} ${metric_name} ${metrics[${metric_name}]} exist. Skipping..."
      else if [ ${model_path} == "/lustrefs/users/runner/checkpoints/huggingface/falcon-h1-34b" ]; then
